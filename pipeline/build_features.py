...
from build_vectors import (
    BASE, UA, fetch_json, PlayerAgg, process_lineups, process_events,
    match_end_seconds, position_group, FEATURES, LABELS,
    MIN_MINUTES, EXCLUDE_GOALKEEPERS,
)
...
FAMILIES = {
    "attacking": [
        "GOALS_P90", "XG_P90", "FINISHING_P90", "ASSISTS_P90",
        "KEY_PASSES_P90", "CROSSES_P90",
    ],
    "passing_control": [
        "PASSES_CMP_P90", "PASS_CMP_PCT", "PROG_CARRY_P90",
        "DRIBBLES_P90", "FOULS_WON_P90",
    ],
    "defending_duel": [
        "PRESSURES_P90", "TACKLES_P90", "INTERCEPTIONS_P90",
        "RECOVERIES_P90", "FOULS_CONV_P90",
    ],
}
...
def build_context(comp_id: int, season_id: int, label: str, cached_only: bool):
    ...
    matches = fetch_json(f"{BASE}matches/{comp_id}/{season_id}.json",
                         f"matches_{comp_id}_{season_id}.json", note=f"{label} match list")
    if not matches:
        return [], 0, 1
    ...
    for pid, a in agg.items():
        ...
        rows.append({
            "player_id": pid, "name": a.name, "team": a.team, "pos": pos,
            "minutes": a.minutes, "context": label, **vals,
        })
...
def main() -> int:
    ...
    ap.add_argument("--out", type=str, default="train_matrix",
    ...
    print(f"mode={mode} | {len(selected)} competition-seasons selected"
    ...
    for comp_id, season_id, label in selected:
        ...
        if rows:
            ...
            total_matches += m_ok
            total_fail += m_fail
            print(f"  {label}: {len(rows)} qualified player-seasons "
    ...
    Z = np.zeros_like(X)
    for ctx, idxs in ctx_index.items():
        block = X[idxs]
    ...
    np.savez(DATA / f"{args.out}.npz",
             X=Z.astype(np.float32), M=M, ctx_ids=ctx_ids,
             n_rows=n, n_features=d)
    (DATA / f"meta_{args.out}.json").write_text(json.dumps([{
        "name": r["name"], "team": r["team"], "pos": r["pos"],
        "minutes": round(r["minutes"], 1), "context": r["context"],
        "player_id": r["player_id"],
    } for r in all_rows], separators=(",", ":")), encoding="utf-8")

    fam_lists = {fam: cols for fam, cols in FAMILIES.items()}
    coverage = {fam: 1.0 for fam in FAMILIES}
    manifest = {
