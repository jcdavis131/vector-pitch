...
def timeline(rec):
    for p in [
        TIMELINE,
        ROOT / "bundles" / "timeline.jsonl",
        pathlib.Path.home() / ".scout" / "missions" / "_cron" / "timeline.jsonl",
    ]:
        try:
...
def stdlib_smoke():
    print(
        "[pitch v4 GraphBFF dual] smoke 3 towers 16→32 ResidualTower LN GELU×2 gated attn TCN k3 d[1,2,4] depth3 receptive7 40% has_form 60% fallback archetype8"
    )
    print(
        "[pitch v4] TCA4 heads same-archetype dominant40% same-league9 difficulty-tier4 same-real-vs-aug sparse softmax per-type fusion0.7/0.3 L2 64→32 masked15% BCE VICReg SupCon"
    )
    print(
        "[pitch v4] KL64 clusters league+formation RR32/type 128 edges pos_cluster0.797→0.84 knn5 0.7894→0.85 nn_role0.7492→0.85 sil0.683→0.75"
    )
    try:
        import json
        ...
        E = rng.standard_normal((633, 32)).astype("float32")
        ...
        timeline(
            {
                "nodeId": "pitch-v4-graphbff-smoke",
                "agentId": "pitch-swarm",
                "attempt": 1,
                "latency_ms": 220,
                "tokens_est": 1600,
