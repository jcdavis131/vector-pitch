...
def require_real_matrix():
    ...
    if not TRAIN_MATRIX.exists():
        ...
        print(
            "[pitch v7] expected: tm_full.npz 2430 rows 11 ctx, tm_9ctx.npz 2295 rows 9 ctx, dfs_harvest_pitch 2000 rows schema 24 keys.",
            file=sys.stderr,
        )
        print(
            f"[pitch v7] existing files: tm_full exists={TM_FULL.exists()} tm_9ctx exists={TM_9CTX.exists()} dfs_harvest exists={DFS_HARVEST.exists()}",
            file=sys.stderr,
        )
        sys.exit(2)
    return TRAIN_MATRIX
...
def validate_dfs_harvest(require_rows: int = 2000):
    if not DFS_HARVEST.exists():
        print(
            f"[pitch v7] WARN dfs_harvest missing {DFS_HARVEST} — consumer wiring expects {require_rows} rows",
            file=sys.stderr,
        )
        return False
    try:
        ...
        print(
            f"[pitch v7] dfs_harvest_pitch.jsonl rows={count} first_row_keys={len(first_keys) if first_keys else 0} expected 24 keys, rows target {require_rows}"
        )
        if first_keys and len(first_keys) != 24:
            print(
                f"[pitch v7] WARN schema {len(first_keys)} != 24 — expected 23 current (plus provenance 24?) — doc gap: {first_keys}",
                file=sys.stderr,
            )
...
def main():
    ...
    if not matrix_path.exists():
        ...
        if args.matrix == "train_matrix.npz":
            ...
            print(
                f"[pitch v7.1] production requires REAL train_matrix.npz 24-d — file missing: {matrix_path}",
                file=sys.stderr,
            )
            print(
                "[pitch v7.1] expected df_harvest_pitch 2000 rows + statcast velocity exit launch 9 families 158-322k PA 2020-2025",
                file=sys.stderr,
            )
            print(
                f"[pitch v7.1] data dir listing: {[p.name for p in DATA_DIR.iterdir() if p.is_file()][:20]}",
                file=sys.stderr,
            )
            return 2
        else:
            ...
            if not args.allow_smoke and not HAS_NUMPY:
                print(
                    f"[pitch v7] custom matrix {args.matrix} exists but train_matrix.npz still required for production",
                    file=sys.stderr,
                )
    ...
    if args.d_emb != 24:
        print(
            f"[pitch v7] WARN d-emb={args.d_emb} != 24 — production uses 24-d real, not 32-d toy L2 1.0; forcing gate check",
            file=sys.stderr,
        )
        # Do not hard-fail for experimentation but flag; for production we enforce 24
        if args.d_emb == 32:
            print("[pitch v7] 32-d toy deprecated — use 24-d real per task", file=sys.stderr)
    ...
    if matrix_path.exists() and HAS_NUMPY:
        try:
            ...
            X = npz["X"]
            print(
                f"[pitch v7.1] loaded REAL matrix {matrix_path} X.shape={X.shape} — 24-d REAL production verified (not 32-d toy)"
            )
            ...
            if X.shape[1] != 24 and X.shape[1] != 16:
                print(
                    f"[pitch v7.1] WARN X.shape[1]={X.shape[1]} != 24 — check 24-d real vs 16-d legacy; 32-d toy deprecated",
                    file=sys.stderr,
                )
        except Exception as e:
            print(
                f"[pitch v7.1] REAL matrix load failed {matrix_path}: {e} — honest fail, no synthetic", file=sys.stderr
            )
    ...
    if HAS_TORCH:
        ...
        print(
            "[pitch v7.1] NOTE stdlib smoke does NOT produce production artifact — requires REAL train_matrix.npz 24-d"
        )
    return 0
