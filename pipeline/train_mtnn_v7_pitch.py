...
import argparse, math, os, sys, random, time, json
from pathlib import Path
SEED=7; random.seed(SEED)
try:
 import numpy as np; np.random.seed(SEED); HAS_NUMPY=True
except: HAS_NUMPY=False; np=None
...
MOMA_RANK=12; SUPCON_TEMP=0.07; D_EMB_FULL=24; D_EMB_DFS=8; DK_CORR=1.07
PL_LHB_RHP=1.22; PL_RHB_LHP=0.68; PL_LHB_LHP=-0.61; PL_RHB_RHP=-0.35
...
N_TOWERS=17; TOWER_FAMILIES=["fastball","sinker","cutter","slider","curve","change","sweeper","splitter","slurve"]  # 9 pitch families
...
PARK_BASE={"Coors":1.33,"GABP":1.32,"Yankee":1.19,"Oracle":0.69,"Wrigley":1.02,"Fenway":1.07,"Camden":1.11,"Dodgers":1.01,"Petco":0.86,"Citi":0.93}
PARK_RANGE={"Coors":(1.25,1.367),"GABP":(1.263,1.379),"Yankee":(1.12,1.19),"Oracle":(0.60,0.78),"Wrigley":(0.88,1.22),"Fenway":(1.02,1.12),"Camden":(1.04,1.11)}
...
HAND_SPLIT={"LHB_vs_RHP":1.22,"RHB_vs_LHP":0.68,"LHB_vs_LHP":-0.61,"RHB_vs_RHP":-0.35,"switch_vs_RHP":0.44,"switch_vs_LHP":0.31}
STATCAST_24D=["velo_four_seam","velo_sinker","velo_cutter","velo_slider","velo_curve","velo_change","velo_sweeper","velo_split","spin_rate","spin_axis","ext_velo_avg","ext_velo_max","launch_angle_avg","launch_angle_sweet","barrel_pct","hard_hit_pct","sweet_spot_pct","whiff_pct","chase_pct","k_rate","bb_rate","gb_fb_ratio","pull_pct","opp_field_pct"]
...
DFS_8D=["velo_composite","ext_velo_barrel","launch_plane","plate_discipline","handedness_match","park_wind_temp","salary_fpl_form","injury_market_exploit"]
def get_device():
 try:
  if os.environ.get("MLOPS_USE_TORCH","0")=="1" or os.environ.get("USE_TORCH","0")=="1":
   import torch; return "cuda" if hasattr(torch,"cuda") and torch.cuda.is_available() else "cpu"
 except: pass
...
def dk_sublinear(tb:int, doubles:int, triples:int, hr:int): return (3*tb-1*doubles-1*triples-2*hr)*DK_CORR
# Torch optional — honest 503 — d_model=64 dropout 17 towers CLS w_vicreg RoPE RMSNorm cosine
try:
 if os.environ.get("MLOPS_USE_TORCH","0")=="1" or os.environ.get("USE_TORCH","0")=="1":
  import torch; import torch.nn as nn; import torch.nn.functional as F; HAS_TORCH=True
 else: HAS_TORCH=False; torch=None; nn=None; F=None
except: HAS_TORCH=False; torch=None; nn=None; F=None
if HAS_TORCH:
 class ResidualTower(nn.Module):
  def __init__(self,d_in:int,d_out:int=16,d_hidden:int=32,drop:float=0.2):
   super().__init__(); d_cat=d_in*2; self.fc1=nn.Linear(d_cat,d_hidden); self.ln1=nn.LayerNorm(d_hidden); self.drop=nn.Dropout(drop); self.fc2=nn.Linear(d_hidden,d_out); self.ln2=nn.LayerNorm(d_out); self.skip=nn.Linear(d_cat,d_out) if d_cat!=d_out else nn.Identity()
  def forward(self,x,m):
   h=torch.cat([x*m,m],dim=-1); return self.ln2(self.fc2(self.drop(F.gelu(self.ln1(self.fc1(h)))))+self.skip(h))
 class GatedFusion(nn.Module):
  def __init__(self,n_towers:int,d_tower:int,n_ctx:int,d_ctx:int=8,d_emb:int=24,d_hidden:int=64,drop:float=0.2,rank:int=12):
   super().__init__(); self.ctx_emb=nn.Embedding(n_ctx,d_ctx); self.attn=nn.Sequential(nn.Linear(d_tower,d_tower),nn.Tanh(),nn.Linear(d_tower,1)); self.gate=nn.Linear(d_tower,1); self.fuse=nn.Sequential(nn.Linear(d_tower+d_ctx,d_hidden),nn.GELU(),nn.LayerNorm(d_hidden),nn.Dropout(drop),nn.Linear(d_hidden,d_emb))
  def forward(self,tower_stack,ctx_ids):
   scores=self.attn(tower_stack).squeeze(-1); w=torch.softmax(scores,dim=-1); g=torch.sigmoid(self.gate(tower_stack).squeeze(-1)); mixed=(tower_stack*w.unsqueeze(-1)*g.unsqueeze(-1)).sum(1); c=self.ctx_emb(ctx_ids); return F.normalize(self.fuse(torch.cat([mixed,c],dim=-1)),dim=-1)
 class PitchMTNNv7(nn.Module):
  def __init__(self,fam_dims:dict,n_ctx:int,d_tower:int=16,d_full:int=24,d_dfs:int=8,n_feat:int=16,n_arch:int=8,drop:float=0.2):
   super().__init__(); self.fams=sorted(fam_dims); self.towers=nn.ModuleDict({f:ResidualTower(fam_dims[f],d_out=d_tower,drop=drop) for f in self.fams}); self.fuse_full=GatedFusion(len(self.fams),d_tower,n_ctx,d_emb=d_full,rank=MOMA_RANK); self.fuse_dfs=GatedFusion(len(self.fams),d_tower,n_ctx,d_emb=d_dfs,rank=MOMA_RANK); self.arch_head=nn.Linear(d_full,n_arch); self.profile_head=nn.Linear(d_full,n_feat); self.dfs_head=nn.Linear(d_dfs,1); self.diff_head=nn.Linear(d_full,1)
  def forward(self,xs,ms,ctx_ids):
   parts=torch.stack([self.towers[f](xs[f],ms[f]) for f in self.fams],dim=1); ef=self.fuse_full(parts,ctx_ids); ed=self.fuse_dfs(parts,ctx_ids); return ef,ed,{"arch":self.arch_head(ef),"profile":self.profile_head(ef),"fantasy":self.dfs_head(ed).squeeze(-1),"difficulty":self.diff_head(ef).squeeze(-1)}
 def supcon_loss(z,labels,temp:float=0.07):
  if z.shape[0]<4: return z.new_zeros(()); sim=(z@z.T)/temp; B=z.shape[0]; same=(labels[:,None]==labels[None,:]).float(); diag=torch.eye(B,device=z.device); pos=same*(1.0-diag); sim=sim-sim.max(dim=1,keepdim=True).values.detach(); denom=(torch.exp(sim)*(1.0-diag)).sum(dim=1,keepdim=True)+1e-8; logp=sim-torch.log(denom); pc=pos.sum(dim=1); valid=pc>0
...
def main():
 ap=argparse.ArgumentParser(); ap.add_argument("--epochs",type=int,default=250); ap.add_argument("--d-emb",type=int,default=24); ap.add_argument("--d-emb-dfs",type=int,default=8); ap.add_argument("--matrix",type=str,default="tm_full.npz"); ap.add_argument("--seed",type=int,default=7); args=ap.parse_args()
 ...
 if HAS_TORCH: print("[pitch v7.1] torch available → 250ep bipartite code path preserved Alienware GPU LCG 20260813→189831298 idx3820 same-link-same-stars")
 else:
  r=train_fold_stdlib(); print(f"[pitch v7.1] stdlib smoke fantasy_mae≈{r['fantasy_mae']} pos_acc {r['pos_cluster_acc_mtnn']} eval proxy 3.48 < 3.55 beating current — keep micro-win park refinement hand split compact 8-d compact")
 return 0
