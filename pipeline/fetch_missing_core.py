...
TOUR_REGIME_BY_YEAR: Dict[str, Dict] = {
    "2018": {"atp_finals_prize": 2_712_000, "wta_finals_prize": 2_360_000, "slam_prize_wimbledon": 2_250_000, "ranking_system": "ATP 2009+ 52wk rolling", "era": "Federer/Nadal/Djokovic + Serena"},
    "2019": {"atp_finals_prize": 2_656_000, "slam_prize_wimbledon": 2_350_000, "ranking_system": "ATP 2009+", "era": "Big3 tail"},
    "2020": {"atp_finals_prize": 1_564_000, "slam_prize_wimbledon": 0, "ranking_system": "ATP COVID freeze 22mo", "era": "COVID ranking freeze"},
    "2021": {"atp_finals_prize": 2_316_000, "slam_prize_wimbledon": 1_700_000, "ranking_system": "ATP thaw", "era": "COVID thaw"},
    "2022": {"atp_finals_prize": 4_740_000, "slam_prize_wimbledon": 2_000_000, "ranking_system": "ATP restored", "era": "Alcaraz breakthrough"},
    "2023": {"atp_finals_prize": 4_801_500, "slam_prize_wimbledon": 2_350_000, "ranking_system": "ATP+", "era": "Djokovic 24"},
    "2024": {"atp_finals_prize": 4_881_100, "slam_prize_wimbledon": 2_700_000, "ranking_system": "ATP+", "era": "Sinner/Alcaraz 1-2"},
    "2025": {"atp_finals_prize": None, "slam_prize_wimbledon": None, "ranking_system": "ATP live", "era": "live fetch required"},
}
...
def audit_cache() -> Dict:
    ...
    return {
        ...
        "cache_years_found": sorted(list(years_found))[:20],
...
def fetch_year_placeholder(year: int, force=False, offline=False) -> bool:
    ...
    if not force and atp_file.exists() and atp_file.stat().st_size > 0 and wta_file.exists() and wta_file.stat().st_size > 0:
        return True
    if offline:
        return False
    if "--scaffold-write" in sys.argv:
        for p in [atp_file, wta_file]:
            if p.exists() and not force:
                continue
            placeholder = {"year": year, "tour": p.stem.split("_")[0], "matches": [], "players": [], "stub": True, "_scaffold": "fetch_missing_core.py placeholder"}
...
def main():
    ...
    if dry_run or (audit_only and "--full" not in args and "--scaffold-write" not in args):
        ...
        print(f"Years missing: {audit['missing_years_count']} / {len(EXPECTED_YEARS)} — expected {audit['coverage_years']}")
        print(f"Skeleton? pitch.json {audit['assets_pitch_bytes']} bytes count={audit['assets_pitch_count']} vectors.json {audit['assets_vectors_bytes']} bytes count={audit['assets_vectors_count']} skeleton={audit['assets_skeleton']}")
