...
def main() -> int:
    ...
    print(
        f"mtnn embeddings: {len(mtnn['players'])} players, d_emb {mtnn['d_emb']}, "
        f"con_w {mtnn.get('config', {}).get('con_w')}"
    )
    ...
    out_game = {
        ...
        "embedding_note": (
            "retrieval is MTNN 24-d L2 (v); map is PCA3 (x,y,z) from original "
            "tournament-z; profile 16-d stored for trait/pitch UI — ablation that "
            "beats PCA3 0.7265 vs 0.7008 per audit"
        ),
        "provenance": (
            f"promoted from {MTNN.name} (n={len(mtnn['players'])}, d_emb=24, "
            f"con_w=0.5, supcon pos_acc 0.797) + {VEC_OLD.name} PCA3 map"
        ),
        ...
        "pca_fallback": {
            "available": True,
            "note": (
                "game.js can fallback to profile 16-d if embedding flag missing; "
                "old vectors.json backed up as vectors_pca3.json if needed"
            ),
        },
    }
    ...
    out_full = {
        ...
        "attribution": (
            "Data: StatsBomb Open Data (statsbomb.com) -- free data license, "
            "attribution required, MTNN v1.1 SupCon 0.5"
        ),
    }
