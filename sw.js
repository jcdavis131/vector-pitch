/* pitch PWA v67 — CORE20 offline13k LOD4000/8000 DPR1 fillRect #080A0F
...
*/
const CACHE_NAME = 'vector-pitch-v67-offline13k';
const CORE = [
'/',
'/index.html',
'/manifest.json',
...
self.addEventListener('install', e=>{
  ...
  e.waitUntil((async()=>{
    ...
    const results=await Promise.allSettled(CORE.map(u=> cache.add(new Request(u,{cache:'reload'})).catch(err=>{ console.warn('[sw v67 13k pitch] miss',u,err&&err.message); return null; })));
    const ok=results.filter(r=>r.status==='fulfilled'&&r.value!==null).length;
    console.log(`[sw v67 pitch] CORE ${ok}/`+CORE.length+` — 20×5888B ≈117k shell 74k gz 13k offline dark card void #080A0F — LOD4000/8000 DPR1 fillRect #080A0F — LCG 20260813→189831298 idx3820 triple[11205,19448,14209] five[11205,19448,14209,11701,18524] same-link-same-stars ?daily=YYYYMMDD&n=1/3/5 — momentum 0.94 k120 b0.18`);
  })());
});
...
self.addEventListener('activate', e=>{
  e.waitUntil((async()=>{
    ...
    console.log('[sw v67 pitch] activate '+CACHE_NAME+' — 74k HIT offline13k CORE20 LOD4000/8000 DPR1 momentum0.94 k120 b0.18 quaternion arcball void #080A0F');
  })());
});
